...
def tsne(H, 
    ...
    self_organized_map_train = model_.fit_transform(Z_train)
    ...
    for _class in classes:
        ...
        ax1.plot(self_organized_map_train[index,0], self_organized_map_train[index,1], '.', label = str(_class))
    plt.legend()
    plt.close()

    self_organized_map_train = np.c_[self_organized_map_train, Y_train]
    ...
    # perform t-SNE
    self_organized_map_test = model_.fit_transform(Z_test)
    ...
    for _class in classes:
        index = Y_test == _class
        ax2.plot(self_organized_map_test[index,0], self_organized_map_test[index,1], '.', label = str(_class))
    plt.legend()
    plt.close()

    self_organized_map_test = np.c_[self_organized_map_test, Y_test]    

    return fig_train, fig_test, self_organized_map_train, self_organized_map_test
